...
export class CredentialsExchangeListComponent {
  @Input() credentialsExchangeRecords: CredentialExchangeRecord[];
  ...
  ngOnInit() {
    ...
    this.credentialsExchangeRecords.forEach(
      (credExRec: CredentialExchangeRecord) => {
        connectionIds.push(credExRec.connection_id);
      }
    );
    const unique = [...new Set(connectionIds)];
    unique.forEach(connId => {
      this.agentService.getConnection(connId).pipe(
        map((el: Connection) => this.uniqueConnections.push(el))
      ).subscribe();
    })
  }
